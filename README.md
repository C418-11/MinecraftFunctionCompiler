...
* [编译](#编译)
  * [1.配置](#1-配置)
    * [1. 配置通用](#11-通用配置)
    * [2. 配置源码编译](#12-配置源码编译)
    * [3. 配置支持包编译](#13-配置支持包编译)
  * [2.编译打包](#2-编译打包)
    * [1. 编译支持包](#21-编译支持包)
    * [2. 编译源码](#22-编译源码)
    * [3. 组合编译后的源码](#23-组合编译后的源码)
...
* 函数的参数目前不支持关键词参数
* Return语句暂时不会提前结束函数 (正在搓了)
* 命名不建议与内置函数名相同(没测试过 行为未知)
* 不建议使用内置函数, 建议使用等价模板函数(例如print的等价函数tprint)
* 基岩版未进行过任何测试
...
* [`导入函数`(点击)](./tests/import_add/caller.py)
* [`From导入函数`(点击)](./tests/from_import_add/caller.py)
* [`赋值导入变量`(点击)](./tests/assign_import_var/caller.py)
...
(引用)[`1.2 配置源码编译`(点击跳转)](#12-配置源码编译)中`main.py`配置的`BASE_NAMESPACE`
